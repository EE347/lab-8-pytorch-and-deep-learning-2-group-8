...
if __name__ == '__main__':
    ...
    for epoch in range(1,5):
        ...
        for images, labels in tqdm(testloader, total=len(testloader), leave=False):
            ...
            # Append the current batch predictions and labels for confusion matrix
        ...
        sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=['Person 1', 'Person 2'], yticklabels=['Person 1', 'Person 2'])
        plt.xlabel('Predicted')
        plt.ylabel('True')
        plt.title(f'Confusion Matrix - Epoch {epoch}')
        
        # Save the confusion matrix to file (overwrite each time)
